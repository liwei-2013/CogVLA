...
from prismatic.models.vit_wrapper_reg import FiLMedPrismaticVisionBackboneAggregator
...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    def __init__(self, config: PrismaticConfig) -> None:
        ...
        if config.vision_aggregate_type == 'moe':
            ...
            assert config.use_fused_vision_backbone, \
                '`use_fused_vision_backbone` should be True when using `moe` for aggregation'
        ...
        if config.text_config.use_lfp:
            print("Using LFP Router...")
            from prismatic.models.modeling_llama import replace_llama_forward
            replace_llama_forward()
        ...
        for layer in self.language_model.model.layers:
            ...
            # layer.self_attn.get_num_patches = self.get_num_patches
            # layer.self_attn.get_num_images_in_input = self.get_num_images_in_input
    ...
    def _aggregate_patch_features(self, patch_features, language_embeddings=None):
        if self.config.vision_aggregate_type == 'moe':
            ...
            patches_llm_proj = self.featurizer_proj(patches)
            patches_fused_llm_proj = self.fused_featurizer_proj(patches_fused)
            image_embeds = self.aggregation_router([patches_llm_proj, patches_fused_llm_proj], average_language_embedding)
        elif self.config.vision_aggregate_type == 'concat':
            image_embeds = torch.cat(patch_features, dim=2)  # Concatenate SigLIP and DINOv2 patches along the hidden dimension
            image_embeds = self.projector(image_embeds)  # Project patch embeddings into language embedding space
        else:
            raise NotImplementedError

        return image_embeds  # (B, L, D)

    def _process_vision_features(self, pixel_values, language_embeddings=None):
        """Process vision features with optional FiLM conditioning"""
        if isinstance(self.vision_backbone, FiLMedPrismaticVisionBackboneAggregator) or isinstance(self.vision_backbone, FiLMedPrismaticVisionBackbone):
        ...
        if self.vision_backbone.get_num_images_in_input() == 1:
            image_embeds = self._aggregate_patch_features(patch_features, language_embeddings)  # (bsz, 256, D)
        else:
            all_image_embeds = []
            ...
            # Concatenate all patches along the patch dimension, (bsz, 256 * num_images, D)
            image_embeds = torch.cat(all_image_embeds, dim=1)

        # if hasattr(self, "text_router"):
        #     image_embeds = self.text_router(image_embeds, language_embeddings)  # for ablation
        return image_embeds
