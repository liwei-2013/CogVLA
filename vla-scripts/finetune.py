...
from prismatic.models.vit_wrapper_reg import FiLMedPrismaticVisionBackboneAggregator, PrismaticVisionBackboneAggregator
...
class FinetuneConfig:
    ...
    use_aggr: bool = False                           # If True, uses aggregation tokens to compress visual tokens
    num_vision_aggr: int = 32
    use_lfp: bool = False                            # If True, uses LFP Routing to compress visual tokens
    lfp_average_factor: float = 0.5
    lfp_type: str = 'shiftedcos_decay_0.85_0.15'
    lfp_enable_film: bool = False                    # If True, enable FiLM for LFP
...
def print_trainable_parameters(model):
    ...
    print(f'total params: {total_param / 1e6} M\ntrainable params: {trainable_param / 1e6} M')
    print("--- NOTE: When using deepspeed zero3, the params count maybe inaccurate ---")
...
def get_peft_state_non_lora(named_params, require_grad_only=True, excluded_names=None):
    to_return = {k: t for k, t in named_params if "lora_" not in k}
    if require_grad_only:
        to_return = {k: t for k, t in to_return.items() if t.requires_grad}

    excluded_names = [] if excluded_names is None else excluded_names
    to_return = {k: t for k, t in to_return.items() if not any(keyword in k for keyword in excluded_names)}
...
def count_parameters(module: nn.Module, name: str) -> None:
    ...
    print(f"# trainable params in {name}: {num_params / 10** 6} M")
...
def save_training_checkpoint(
    ...
    if cfg.use_lora and cfg.merge_lora_during_training:
        # setting low_cpu_mem_usage=True will cause the new module being placed at meta device
        base_vla = AutoModelForVision2Seq.from_pretrained(
            cfg.vla_path, config=vla_config, torch_dtype=torch.bfloat16, low_cpu_mem_usage=False, trust_remote_code=True
        )

        # ================== NEW ==================
        non_lora_trainables = get_peft_state_non_lora(
            vla.named_parameters(), excluded_names=['vision_backbone']
        )
        if len(non_lora_trainables.keys()) > 0:
            non_lora_trainables = {
                k.replace("module.base_model.model.", ""): v
                for k, v in non_lora_trainables.items()
            }
            msg = base_vla.load_state_dict(non_lora_trainables, strict=False)
            print(f"Merging non-lora trainable keys: {non_lora_trainables.keys()}")
            print("Non-lora trainables Unexpected_keys:", msg.unexpected_keys)
        # =========================================
        ...
        # ================== NEW ==================
        vla_config.save_pretrained(checkpoint_dir)
        non_lora_trainables = get_peft_state_non_lora(
            vla.named_parameters(), excluded_names=['vision_backbone']
        )
        if len(non_lora_trainables.keys()) > 0:
            ...
            torch.save(non_lora_trainables, checkpoint_dir / f"non_lora_trainables--{checkpoint_name_suffix}")
        # =========================================
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    vla_cfg.text_config.lfp_type = cfg.lfp_type
    vla_cfg.text_config.lfp_enable_film = cfg.lfp_enable_film

    vla_cfg.use_aggr = cfg.use_aggr
    vla_cfg.use_film = cfg.use_film
    vla_cfg.num_vision_aggr = cfg.num_vision_aggr
    vla_cfg.vision_aggregate_type = cfg.vision_aggregate_type
    ...
    vla = AutoModelForVision2Seq.from_pretrained(
        ...
        low_cpu_mem_usage=False,  # setting True will cause the new module being placed at meta device
    ...
    if cfg.use_lora:
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(
                vla, excluded_names=[
                    'featurizer_proj', 'fused_featurizer_proj', 'aggregation_router', 'router', 'scale', 'shift'
                ]),  # NEW
            init_lora_weights="gaussian",
        )
        vla = get_peft_model(vla, lora_config)

        if hasattr(vla.model, 'aggregation_router'):    # NEW
            print("Unfreezing MoE components ...")
        ...
        if vla.config.text_config.use_lfp:
            print("Unfreezing LFP Router components ...")
    ...
    if cfg.use_film or cfg.use_aggr:
        ...
        if cfg.use_aggr:
            vision_wrapper = FiLMedPrismaticVisionBackboneAggregator if cfg.use_film else PrismaticVisionBackboneAggregator
            vla.model.vision_backbone = vision_wrapper(
                vision_backbone=vla.model.vision_backbone,
                llm_dim=vla.llm_dim,
                num_vision_aggr=NUM_VISION_AGGR
            )
        else:
            # cfg.use_film and not cfg.use_aggr
        ...
        # ============ resume ============
        # state_dict = load_checkpoint("vision_backbone", cfg.vla_path, 70000)
        # state_dict = {k: v for k, v in state_dict.items() if 'scale' in k or 'shift' in k or 'vision_queries' in k}
        # print(state_dict.keys())
        # msg = vla.model.vision_backbone.load_state_dict(state_dict, strict=False)
        # print('vit film:', msg.unexpected_keys)
    ...
    if cfg.use_proprio:
        ...
        # ============ resume ============
        # state_dict = load_checkpoint("proprio_projector", cfg.vla_path, 150000)
    ...
    if cfg.use_l1_regression:
        ...
        # ============ resume ============
        # state_dict = load_checkpoint("action_head", cfg.vla_path, 150000)
    ...
    if not cfg.use_aggr:
        ...
        NUM_PATCHES = NUM_VISION_AGGR * vla.module.vision_backbone.get_num_images_in_input()
    ...
    print(f"# total trainable params: {sum(p.numel() for p in trainable_params) / 10**6} M")
    ...
    warmup_step = 2000
    warmup_scheduler = LambdaLR(optimizer, lambda cur_step: cur_step / warmup_step)
    multistep_scheduler = CosineAnnealingLR(
        optimizer,
        T_max=80_000,
        eta_min=1e-8,
    )
    scheduler = SequentialLR(optimizer, [warmup_scheduler, multistep_scheduler], milestones=[warmup_step])
