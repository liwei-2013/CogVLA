...
def main(cfg: ConvertConfig) -> None:
    ...
    vla_cfg = AutoConfig.from_pretrained(cfg.lora_finetuned_checkpoint_dir)
    ...
    # ================ NEW ================
    for fname in os.listdir(cfg.lora_finetuned_checkpoint_dir):
        if "non_lora_trainable" in fname:
            non_lora_trainables = torch.load(os.path.join(cfg.lora_finetuned_checkpoint_dir, fname))
            msg = vla.load_state_dict(non_lora_trainables, strict=False)
