...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    lora_cfg = json.load(open(os.path.join(cfg.pretrained_checkpoint, 'lora_adapter/adapter_config.json')))
    cfg.lora_rank = lora_cfg["r"]
    cfg.lora_alpha = lora_cfg["lora_alpha"]
...
def _apply_film_to_vla(vla: torch.nn.Module, cfg: Any) -> torch.nn.Module:
    ...
    lora_config = LoraConfig(
        ...
        target_modules=find_all_linear_names(
            vla, excluded_names=['featurizer_proj', 'fused_featurizer_proj', 'aggregation_router', 'router', 'language_model']),
    ...
    if vla.config.use_aggr:
        vision_wrapper = FiLMedPrismaticVisionBackboneAggregator if vla.config.use_film else PrismaticVisionBackboneAggregator
        new_vision_backbone = vision_wrapper(
...
def get_proprio_projector(cfg: Any, llm_dim: int, proprio_dim: int) -> ProprioProjector:
    ...
    if model_is_on_hf_hub(cfg.pretrained_checkpoint):
        model_path_to_proprio_projector_name = {
            "todo/cogvla": "proprio_projector--150000_checkpoint.pt",
        }
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead]:
    ...
    if model_is_on_hf_hub(cfg.pretrained_checkpoint):
        model_path_to_action_head_name = {
            "todo/cogvla": "action_head--150000_checkpoint.pt",
        }
